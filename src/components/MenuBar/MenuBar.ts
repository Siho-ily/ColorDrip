...
export default class MenuBar {
    ...
    constructor({
        ...
        onSettingsToggle: (anchorRect: DOMRect) => void;
    }) {
        ...
        this.$el.append(this.$rainBtn, this.$sep1, this.$paletteBtn, this.$darkBtn, this.$sep2, this.$settingsBtn);
        ...
        this.$settingsBtn.addEventListener('click', () =>
            onSettingsToggle(this.$settingsBtn.getBoundingClientRect())
        );

        this.$el.addEventListener('mouseenter', () => this.showBar());
        document.addEventListener('mousemove', () => this.showBar(), { passive: true });
    ...
    private applySnap(edge: SnapEdge, withTransition: boolean) {
        ...
        switch (edge) {
            case 'top':    x = (vw - bw) / 2;           y = SNAP_MARGIN;             break;
            case 'bottom': x = (vw - bw) / 2;           y = vh - bh - SNAP_MARGIN;   break;
            case 'left':   x = SNAP_MARGIN;              y = (vh - bh) / 2;           break;
            case 'right':  x = vw - bw - SNAP_MARGIN;   y = (vh - bh) / 2;           break;
        }
    ...
    private scheduleFade() {
        ...
        this.fadeTimer = setTimeout(() => {
            ...
            animate(this.$el, { opacity: 0 }, { duration: 0.6 });
        }, FADE_DELAY);
    }
